...
public:
    ...
    void move() // Обновление фреймов и вычисление нажатой клавиши перемещения.
    {
        if (!isAttack && Health) {
            ...
            if (Keyboard::isKeyPressed(Keyboard::Left)) // Бег вправо.
            {
                isRight = false;
                DirPlus(1 << 1); setSpeed(0.1);
                sprite.setTextureRect(IntRect(32 * int(CurrentFrame) + 32, 64, -32, 32));
            }
            if (Keyboard::isKeyPressed(Keyboard::Right)) // Бег влево.
            {
                isRight = true;
    ...
    void interactionWithMap(float& time, sf::String TileMap[])
    {
        for (int i = 0; i < h; i++)
        {
            ...
            if ((dx < 0) && TileMap[int(x + dx) / TILE_SIZE][int(y + i) / TILE_SIZE] == '0') // Проверка пересикаечения со стеной при перемещении вправо.
            {
        ...
        for (int j = 0; j < w; j++)
        {
            if ((dy > 0) && TileMap[int(x + j) / TILE_SIZE][int(y + h + dy) / TILE_SIZE] == '0') // Проверка пересикаечения со стеной при перемещении вниз.
            {
                y = int(y + dy);
                break;
            }
            if ((dy < 0) && TileMap[int(x + j) / TILE_SIZE][int(y + dy) / TILE_SIZE] == '0') // Проверка пересикаечения со стеной при перемещении вверх.
            {
